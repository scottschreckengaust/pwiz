...
namespace pwiz.Skyline.Model.AuditLog
{
    ...
    public abstract class DiffNode
    {
        ...
        protected virtual string ObjectToString(bool allowReflection, object obj, out bool isName)
        {
            ...
            if (usesReflection)
                return !allowReflection ? null : text;

            return text.StartsWith("{") && text.EndsWith("}") ? text : LogMessage.Quote(text); // Not L10N
        }

        public DiffNodeNamePair FindFirstNotOneChildNode(DiffTree tree, PropertyName name, bool shortenName, bool allowReflection, DiffNode parentNode = null)
        {
            var oneNode = Nodes.Count == 1;

            // The root property has no name, so we immediately go to the first child node
            if (Property.IsRoot)
                return oneNode ? Nodes[0].FindFirstNotOneChildNode(tree, null, shortenName, allowReflection, this) : null;

            var propertyNameString = Property.GetName(tree, parentNode != null ? parentNode.Objects.FirstOrDefault() : null);
            ...
            if (shortenName && Property.ElementName != null && !IsCollectionElement)
            {
                if (oneNode)
                {
                    ...
                    newName = PropertyName.Root.SubProperty(new PropertyElementTypeName(Property.ElementName));
                }
                else
                {
            ...
            var objects = Objects.Select(o => AuditLogObject.GetAuditLogObject(o))
                .Where(o => o == null || o.IsName).ToArray();
            if (objects.Length == 2)
                if ((objects[0] != null) != (objects[1] != null) || (objects[0] != null && objects[1] != null && objects[0].AuditLogText != objects[1].AuditLogText))
            ...
            return oneNode ? Nodes[0].FindFirstNotOneChildNode(tree, newName, shortenName, allowReflection, this) : new DiffNodeNamePair(this, newName, allowReflection);
        }
        ...
        public List<DiffNodeNamePair> FindAllLeafNodes(DiffTree tree, PropertyName name, bool allowReflection, DiffNode parentNode = null)
        {
            ...
            if (Property.IsRoot)
                return noNodes ? null : Nodes.SelectMany(n => n.FindAllLeafNodes(tree, null, allowReflection, this)).ToList();
            ...
            var canIgnoreName = (Property.IgnoreName && !IsCollectionElement);/* ||
                                (IsCollectionElement && !AuditLogObject
                                     .GetAuditLogObject(Objects.FirstOrDefault(o => o != null)).IsName && noNodes);*/
            ...
            // If the node can't be displayed or ignored, end recursion
            if ((propName.Name == null && !canIgnoreName))
            {
                System.Diagnostics.Debugger.Break(); // TODO: impossible since propName.Name can't be null since we always allow reflection here?
                result.Add(new DiffNodeNamePair(parentNode, name, allowReflection));
                return result;
            }
            // We can't display sub changes of an element if it's unnamed, so we display it's 
            // string representation as a change
            else if (IsCollectionElement && (!isName || isUnnamedNewObjChange || canIgnoreName))
            {
    ...
    public class ElementDiffNode : DiffNode
    {
        ...
        public override LogMessage ToMessage(PropertyName name, LogLevel level, bool allowReflection)
        {
            if (level == LogLevel.undo_redo && Property.ElementName != null)
            {
